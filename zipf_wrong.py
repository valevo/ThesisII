...
from data.corpus import Articles, Words
...
#    
#    default_func = lambda d: len(d)+1 #rand.randint(len(d)+1, len(d)*2)
#    ranks_joined = join_stats(ranks) #, default_func)
...
#    
#    freqs = [compute_normalised_freqs(sub) for sub in subsamples2]
##    mean_freqs = {w: np.mean(fs)  for w, fs in pool_zipf(freqs).items()}
#    
#    default_func = lambda d: 1
#    freqs_joined = join_stats(freqs)#, default_func)
